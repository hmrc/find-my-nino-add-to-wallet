...
class GooglePassControllerSpec extends AnyWordSpec with Matchers with MockitoSugar with BeforeAndAfter {
  ...
  "createPass" must {
    "should return OK with the uuid of the pass" in {
      when(mockGooglePassService.createPass(eqTo("TestName TestSurname"), eqTo("AB 12 34 56 Q"), any())(any()))
  ...
  "getPassCardByPassId" must {
    "should return OK with the byte data of pass" in {
      when(mockGooglePassService.getPassCardByPassId(eqTo(passId))(any()))
