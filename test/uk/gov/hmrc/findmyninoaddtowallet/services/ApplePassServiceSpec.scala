...
import uk.gov.hmrc.findmyninoaddtowallet.repositories.{ApplePass, ApplePassRepository}
...
class ApplePassServiceSpec extends AsyncWordSpec with Matchers with MockitoSugar with BeforeAndAfterEach {
  ...
  "findQrCodeByPassId" must {
    "return the QR Code when pass id exist" in {
      ...
      val pass = new ApplePass(passId, "Test Name", "AB 12 34 56 Q", applePassCard, qrCode, DateTime.now())
  ...
  "createPass" must {
    "should not return an uuid when 'Create Directory' failed" in {
      ...
      val eitherResult = applePassService.createPass("TestName TestSurname", "AB 12 34 56 Q")
