...
class SignatureServiceSpec extends AsyncWordSpec with Matchers with MockitoSugar with BeforeAndAfterEach {
  ...
  "createSignatureForPass" must {
    "return false when a signature was created but an incorrect path was provided" in {
      val path = Files.createTempDirectory(s"${SignatureService.SIGNATURE_FILE_NAME}.pass").toAbsolutePath
      ...
      val pass = signatureService.createSignatureForPass(path, privateKey, "test123", publicKey)
      pass mustBe false
    }
  }
}
