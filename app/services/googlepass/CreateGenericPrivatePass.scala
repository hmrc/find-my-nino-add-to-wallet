...
import services.googlepass.googleModels.{GenericPrivatePass, Image, ImageUri, LocalizedString, TextModuleData, TranslatedString}
...
class CreateGenericPrivatePass @Inject() (config: AppConfig) {
  ...
  private def createAndSignJWT(
    ...
    claims.put("origins", Collections.singletonList(config.googlePassOrigins))
